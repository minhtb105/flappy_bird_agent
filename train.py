...
def train_loop():
    ...
    for episode in range(NUM_EPISODES):
        ...
        if episode % VISUALIZATION_INTERVAL == 0 and episode != 0:
            ...
            plot(scores[::window_size], save_path=f"{vis_folder}/scores.png", label="Score", title="Score per Episode")
            ...
            plot_losses(agent.losses, window_size, save_path=f"{vis_folder}/losses.png")
...
def test_loop(agent, NUM_EPISODES=100, MAX_STEPS_PER_EPISODE=1000, window_size=10):
    ...
    for episode in range(NUM_EPISODES):
        ...
        if (episode + 1) % window_size == 0:
            ...
            print(f"[TEST] Episode {episode+1} - Score: {score}, Mean Score: {mean_score:.2f}, Max Score: {max_score}")

    print("\nTesting completed.")
    print(f"Max score achieved during testing: {max_score}")
    pygame.quit()
